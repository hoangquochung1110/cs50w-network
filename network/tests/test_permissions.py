...
from network.factories import UserFactory, PostFactory
...
class IsOwnerTest(TestCase):
    ...
    def test_is_owner_true(self):
        self.assertTrue(self.permission.has_object_permission(self.request, self.view, self.post_1))

    def test_is_owner_false(self):
        self.assertFalse(self.permission.has_object_permission(self.request, self.view, self.post_2))
...
class FollowPermissionTest(TestCase):
    ...
    def test_request_user_follow_target_user_success(self):
        self.assertTrue(self.permission.has_object_permission(self.request, self.view, self.target_user))

    def test_request_user_follow_target_user_fail(self):
        self.target_user.followers.add(self.request_user)
        self.assertFalse(self.permission.has_object_permission(self.request, self.view, self.target_user))
